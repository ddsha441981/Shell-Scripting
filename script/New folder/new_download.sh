...
# Function to remove Windows-style line endings (if any)
remove_windows_line_endings() {
  # Check if the script is running in a Windows environment (using `uname`)
...
main() {
  ...
  # GitHub token (ensure this token is securely stored in environment variables in a real-world scenario)
  local github_token="YOUR-PAT-TOKEN"  # Set your GitHub token in the environment variable

  # Call the remove_windows_line_endings function (for Windows environments)
